...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        if who == 0:
            gain_score = score0 - previous_score
            current_score = score0
        elif who == 1:
            gain_score = score1 - previous_score
            current_score = score1
        if gain_score > previous_high:
            if gain_score == 1:
                print('1 point! That\'s the biggest gain yet for Player' , who)
            else:
                print(gain_score, 'points! That\'s the biggest gain yet for Player', who)
            return announce_highest(who, gain_score, current_score)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for i in range(10):
        if roll(i+1,dice) > highest:
            highest = roll(i+1,dice)
            dice_nums = i+1
    return dice_nums
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if num >=margin:
        return 0
    if is_swap(score+num, opponent_score):
